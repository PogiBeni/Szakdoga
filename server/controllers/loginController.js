...
async function loadData(req, res) {
  ...
  try {
    const results = await new Promise((resolve, reject) => {
      ...
      connection.query("SELECT * FROM user WHERE email = ? and id = ?", [data.email,data.id], (err, results) => {
        if (err) {
          console.error('Error executing query:', err);
          reject(err);
          return;
  ...
  `,
    ...
    res.status(500).json({ message: 'An error occurred during loading data' });
  }
}

module.exports = { loginUser, loadData };
